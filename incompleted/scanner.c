...
Token* readNumber(void) {
  ...
  int count = 0;
  int flag = 1; // int : flag  =  1, double flag == 0

  while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT || currentChar == '.')) {
  ...
  if(flag < 0) {
    ...
    error(ERR_INVALID_FLOAT, token->lineNo, token->colNo);
    return token;
  }
  token->string[count] = '\0';
  if(flag) token->value = atoi(token->string);
  else {
...
Token *readString(){
  ...
  token->pString = (char*)malloc(1000);
  readChar();
  while(currentChar != EOF && currentChar != '"'){
    token->pString[i++] = currentChar;
    readChar();
  }
  if (currentChar == EOF) {
      token->tokenType = TK_NONE;
      error(ERR_END_OF_STRING, token->lineNo, token->colNo);
      return token;
  }
  token->pString[i] = '\0';
  readChar();
...
void printToken(Token *token) {
  ...
  case KW_STRING: break;
  case KW_DOUBLE: break;
  case KW_RETURN: printf("KW_RETURN\n"); break;
  case KW_REPEAT: break;
  case KW_UNTIL: break;
